...
AVAILABLE_TOOLS = {
    ...
    "query-qhst-journal": {
        "description": "Query QHST journal for sign-on/sign-off events filtered by date and user to track login activity",
        "use_when": ["user logged in", "login history", "sign-on events", "session activity", "who signed on", "login times"],
        "sql_template": """
            SELECT JOURNAL_ENTRY_TIME, JOURNAL_USER, JOURNAL_CODE,
                   JOURNAL_ENTRY_DATA
            FROM TABLE(QSYS2.DISPLAY_JOURNAL(
                'QSYS', 'QHST',
                STARTING_TIMESTAMP => CURRENT_TIMESTAMP - 1 DAY
            ))
            WHERE JOURNAL_CODE IN ('C', 'D')
              AND DATE(JOURNAL_ENTRY_TIME) = CURRENT_DATE
            ORDER BY JOURNAL_ENTRY_TIME DESC
